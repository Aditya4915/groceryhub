...
const CartList = () => {
  ...
  const getSaving = (name: string) => {
    const item = offers.find(
      (offer) => offer.name.toLocaleLowerCase() === name.toLocaleLowerCase(),
    );
  ...
  return (
    <div className="w-1/2 h-1/2 px-6 shadow-xl rounded-lg flex flex-col items-center overflow-scroll scrollbar-none">
      ...
      {cartItems.length === 0 ? (
        ...
        cartItems.map((item) => (
          <div
            className="w-full h-16 mb-2 border rounded-lg flex"
            key={item.id}
          >
            <div className="w-1/3 p-2 gap-2 flex items-center ">
              <img
                src="img.jpg"
                alt="No image"
                className="h-12 w-12 bg-indigo-200"
              />
            ...
            <div className="w-2/3 p-3 flex gap-6 items-center justify-end">
              <div className=" flex flex-col text-xs ">
                ...
                {getSaving(item.name) === 0 ? (
                  <></>
                ) : (
                  <p className=" text-red-500 font-semibold">
                    Savings £{getSaving(item.name)}
                  </p>
                )}
                <p className="font-semibold">
                  Item cost £{" "}
                  {item.price * item.quantity - getSaving(item.name)}
                </p>
              </div>
